class View {
    ...
    constructor() {
        ...
        View.STATE_ALL = "all";
        View.STATE_ACTIVE = "active";
        View.STATE_COMPLETED = "completed";
        ...
        this.$filterAll = document.getElementById("filter_" + View.STATE_ALL);
        this.$filterActive = document.getElementById("filter_" + View.STATE_ACTIVE);
        this.$filterComplete = document.getElementById("filter_" + View.STATE_COMPLETED);
    }

    bindFilterAll(handler) {
        this.$filterAll.addEventListener('click', ({ target }) => {
            handler(View.STATE_ALL);
        });
    }

    bindFilterActive(handler) {
        this.$filterActive.addEventListener('click', ({ target }) => {
            handler(View.STATE_ACTIVE);
        });
    }

    bindFilterComplete(handler) {
        this.$filterComplete.addEventListener('click', ({ target }) => {
            handler(View.STATE_COMPLETED);
        });
    }
    ...
    showItems(items) {
        ...
        for (let item of items) {
            ...
            clone.querySelector("label").innerHTML = View.escapeForHTML(item.name);
